...
namespace hackovic.DbInfo
{
    ...
    public delegate void OpenTableFilteredDelegate(
        object sender, string tableName, bool isDefinitionShown, TableFilterData cellInfo);
    ...
    public partial class ColumnDependencies : Form
    {
        public ColumnDependencies()
        {
            ...
            {
                m_textBox_SpSearch.Text =
                    m_UserControlFullNameListSPNames.SelectedName;
                CreateSpTabPage(m_textBox_SpSearch.Text.Trim());
            };

            m_UserControlFullNameListTableNames.NameSelected += delegate
            {
                m_TextBoxTableName.Text =
                    m_UserControlFullNameListTableNames.SelectedName;
                GetAllTableRows();
            };
        }
        ...
        {
            get { return m_TextBoxTableName.Text.Trim(); }
        }

        protected bool OpenQueryForm { get; set; }
        ...
        public static bool TableExists(string tableName)
        {
            try
            {
                ...
                const string sqlStr =
                    @"SELECT CASE WHEN EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME LIKE @TableName ) THEN cast(1 as BIT) ELSE cast(0 as BIT) END";
                var cmd = new SqlCommand(sqlStr, ConnectionFactory.Instance);
        ...
        public void FillAutoCompleteCustomSource()
        {
            ...
            // 
            var sqlStr =
                @"SELECT DISTINCT TB.TABLE_NAME  FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS TB  ORDER BY TB.TABLE_NAME";

            var ds = FillDataSet(sqlStr);

            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                var dt = ds.Tables[0];
                var tabStrs = new AutoCompleteStringCollection();
            ...
            {
                var dt = ds.Tables[0];
                var tabStrsSp = new AutoCompleteStringCollection();
                foreach (DataRow row in dt.Rows)
                {
                    try
                    {
        ...
        public UserControlAllTableInfo GetSelectedAllTableInfo()
        {
            try
            {
                var tp = tabControl_TableInfo.SelectedTab;
        ...
        private void OpenTableFilteredTab(object sender, string tableName, bool isDefinitionShown, TableFilterData cellInfo)
        {
            if (tableName == null)
            {
                return;
        ...
        private void OpenTableTab(object sender, string tableName, bool isDefinitionShown)
        {
            if (tableName == null)
            {
                return;
        ...
        private void OpenSpTab(object sender, string spName)
        {
        ...
        public UserControlAllTableInfo GetAllTableInfoByTableName(string tableName)
        {
            try
            {
                var tp = GetTabPage(tableName, null);
        ...
        public void CreateTabPage(string tableName, TableFilterData filter)
        {
            if (tabControl_TableInfo.TabPages.ContainsKey(tableName))
            {
                ...
                try
                {
                    ...
                    var allInfo = tabControl_TableInfo.SelectedTab.Controls[0] as UserControlAllTableInfo;
                    if (allInfo != null)
                    {
            ...
            {
                var allInfo = new UserControlAllTableInfo { Dock = DockStyle.Fill };
        ...
        public void CreateTabPageWithValues(string tableName, TableFilterData filter)
        {
            try
            {
                var tp = GetTabPage(tableName, filter);
                tabControl_TableInfo.SelectedTab = tp;
        ...
        private void CloseTabPage(string tabPageKey)
        {
        ...
        /// <summary>
        ///     Handles Tab selecting event.
        ///     When clicked on Query tab, do not change tab just open new <see cref="FormQuery" />
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void OnTabControlSearchSelecting(object sender, TabControlCancelEventArgs e)
        {
            OpenQueryForm = false;
            if (e.TabPage != m_tabPageQuery)
            {
                return;
            }
            OpenQueryForm = true;
        ...
        private void OnTabControlSearchMouseUp(object sender, MouseEventArgs e)
        {
            if (OpenQueryForm)
            {
