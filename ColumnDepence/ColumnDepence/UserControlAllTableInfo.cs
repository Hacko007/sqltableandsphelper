    ...
    public partial class UserControlAllTableInfo : UserControl
    {
        ...
        private void ArrangeDataGridViews()
        {
            try
            {
                ...
                var szSum = FreeSpace(m_DataGridViewChild)
        ...
        private void FillColumnDataGrid()
        {
            ...
            else
            {
                ...
                for (var i = 0; i < m_DataGridViewColumns.Columns.Count; i++)
                {
                ...
                {
                    var refType =
                        TableInfo.ColumnConstrains.GetColumnRefType(
                            row.Cells[TableInfo.ColumnInfo.ColumnColumnName.ColumnName].Value.ToString());
        ...
        {
            var sqlCmd = @"SELECT  count(*) FROM  " + TableName;
            try
            {
        ...
        private void FillDataGridColumnConstrains()
        {
            ...
            else
            {
                ...
                foreach (DataGridViewRow row in m_DataGridViewColumnConstrains.Rows)
                {
                    var refType =
                        TableInfo.ColumnConstrains.GetColumnRefType(
                            row.Cells[TableInfo.ColumnConstrains.ColumnColumnName.ColumnName].Value.ToString());
                    row.HeaderCell = new RowHeaderCellColumnInfo { ColumnRefType = refType };
                }
                m_DataGridViewColumns.AutoResizeRowHeadersWidth(DataGridViewRowHeadersWidthSizeMode.AutoSizeToAllHeaders);
            }
        ...
        private void ShowTableValues_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var c = tool.SourceControl;
                var row = (((DataGridView)(c))).CurrentRow;
                if (row != null)
                {
                    var table = row.Cells[0].Value.ToString();
        ...
        private void ToolStripButtonToolBox_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                var tp = m_ParentForm.GetTabPage(TableName, null);
        ...
        {
            var loadAll = m_ToolStripSplitButtonLoadMain.Text == m_LoadAllValuesToolStripMenuItem.Text;
        ...
        public void DeleteRowFromDb(DataGridViewRow dataGridViewRow)
        {
            ...
            try
            {
                ...
                // 
                var cmdStr = "DELETE FROM " + TableName + " WHERE ";
        ...
        {
            var selColumns = GetSelectedColumnNames();
            var dvFilter = "";

            foreach (var column in selColumns)
            {
                if (dvFilter != "")
                {
                    dvFilter += " OR ";
        ...
        private void ToolStripMenuItem_ShowDefinition_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var val = m_DataGridViewSp.SelectedCells[0].Value.ToString();
                    var isSp = "SP" == m_DataGridViewSp.SelectedCells[0].OwningRow.Cells["MyType"].Value.ToString();
